      ...
      <main className="flex-1 text-black min-h-full bg-gray-200 relative px-4 py-6">
        ...
        <section className="mt-4">
          {incomes.length === 0 ? (
            ...
            <ul>
              {incomes.map((inc) => (
                <li key={inc._id} className="bg-white rounded p-3 mb-2 shadow border-green-500 border-r-4 flex justify-between items-center">
                  <span><strong>{inc.amount} {inc.currency}</strong> – {inc.category}</span> 
                    <span className='font-light text-sm flex'>{String(day).padStart(2, '0')}-{String(month).padStart(2, '0')}-{year}<img src={deleteIcon}className='w-5 h-5 ml-3 cursor-pointer' onClick={()=>handleDeleteIncome(inc._id)}/></span>
